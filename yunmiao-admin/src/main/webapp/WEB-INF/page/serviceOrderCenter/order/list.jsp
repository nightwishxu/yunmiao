...
<script type="text/javascript">
    ...
    var addFun = function() {
        var dialog = parent.sy.modalDialog({
            ...
            width : 800,
            height : 600,
            url : sy.contextPath + '/go.do?path=serviceOrderCenter/order/edit&goodsSource='+goodsSource,
            buttons : [ {
                text : '确定',
                handler : function() {
    ...
    var editFun = function() {
        ...
        var dialog = parent.sy.modalDialog({
            ...
            width : 800,
            height : 600,
            url : sy.contextPath + '/go?path=serviceOrderCenter/order/edit&id=' + rows[0].id,
            buttons : [ {
                text : '确定',
                handler : function() {
                    dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	$(function() {
		grid = $('#grid').datagrid({
			url : sy.contextPath + '/order/orderList?goodsSource='+goodsSource,
			columns : [ [ {
                    ...
                    if(r.id){
                     	if(v != 3){
                            return '<a href="javascript:void(0);" onclick="delivery(\''+r.id+'\');" class="button button-warning" title="发货">发货</a>';
                     	    ...
                     	    return  '<a href="javascript:void(0);" class="button button-default" title="发货">发货</a>';
						}
                    }
                }
            },{
    ...
    function check(id) {
        var dialog = parent.sy.modalDialog({
            ...
            width : 800,
            height : 600,
            url : sy.contextPath + '/go.do?path=serviceOrderCenter/order/detils&id='+id,
        });
    };
