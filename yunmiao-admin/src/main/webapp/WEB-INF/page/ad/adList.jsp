...
<script type="text/javascript">
	...
	var addFun = function() {
		var dialog = parent.sy.modalDialog({
			...
			width : 800,
			height : 600,
			url : sy.contextPath + '/go?path=ad/adEdit&location='+l,
			buttons : [ {
				text : '添加',
				handler : function() {
	...
	var editFun = function() {
		...
		var dialog = parent.sy.modalDialog({
			...
			width : 800,
			height : 600,
			url : sy.contextPath + '/go?path=ad/adEdit&location=' + l + '&id=' + rows[0].id,
			buttons : [ {
				text : '编辑',
				handler : function() {
					dialog.find('iframe').get(0).contentWindow.submitForm(dialog, grid, parent.$);
	...
	var infoFun = function (id){
		var dialog = parent.sy.modalDialog({
			...
			width : 640,
			height : 500,
			url :sy.contextPath +'/detail?id=' +id+'&type=10'
		});
	};

	$(function() {
		grid = $('#grid').datagrid({
			url : sy.contextPath + '/ad/list?location='+l,
			singleSelect : false,
			frozenColumns : [ [ {
				width : '100',
				checkbox:true,
